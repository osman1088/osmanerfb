...
try:
	...
	print W + ' '
	print ('OSM4N3R').center(44)
	print ' '
	print "[!] Modul kurulu degil! 'requests'\n"
	sys.exit()
####################################################################
#                    Varsayilan Kodlama Ayarla
reload (sys)
...
def baliho():
	try:
		...
		print 'ooO--(_)--Ooo'.center(44)
		print ' ' + W
		print ('BILGI').center(44)
		print (W + '     [' + G +'[ OSM4N3R ] Facebook Bilgi'+ W + ']')
		print ' '
####################################################################
#		    Print In terminal
def show_program():
...
* Program OSM4N3R tarafindan dizayn edilmistir.

'''%(G,W)
def info_ga():

	print '''
     %sKOMUTLAR                      ACIKLAMA%s
  -------------       -------------------------------------

   get_data           tum arkadas verilerini aliniyor
   get_info           arkadasin hakkinda bilgi goster

   dump_id            arkadas listesinden tum kimligi aliniyor
   dump_phone         arkadas listesinden tum telefon numaralarini getirme
   dump_mail          arkadas listesinden tum e-postalar getiriliyor
   dump_<id>_id       arkadaslarinizdan tum kimligi getirme <spesific>
		      ex: dump_username_id

   token              Erisim kodu olustur
   cat_token          Erisim kodunuzu gosterin
   rm_token           Erisim kodunu kaldır

   bot                bot menusunu ac

   clear              acik terminal
   help               yardimi goster
   about              bu program hakkinda bilgi goster
   exit               programdan cik
'''%(G,W)
def menu_bot():
	print '''
 ...
 ---------   ------------------------------------
   ...
   [ 01 ]      otomatik reaksiyonlar
   [ 02 ]      otomatik yorum
   [ 03 ]      otomatik durtme
   [ 04 ]      tum arkadaslik isteklerini kabul et
   [ 05 ]      zaman cizelgenizdeki tum yayinlari sil
   [ 06 ]      tum arkadaslari sil
   [ 07 ]      tum arkadaslari takip etmeyi bırak
   [ 08 ]      tum fotograf albumlerini sil
...
def get(data):
	...
	try:
		...
		b.close()
		print '[*] basarıyla erisim belirteci olusturuldu'
		print '[*] Erisim simgeniz cookie/token.log'
		exit()
	except KeyError:
		print '[!] Erisim belirteci olusturulamadi'
		print '[!] Baglantinizi / e-postanizi veya sifrenizi kontrol edin'
		...
		main()
def id():
	print '[*] facebook hesabiniza giris yapin         ';id = raw_input('[?] Kullanici adi : ');pwd = getpass.getpass('[?] Parola : ');API_SECRET = '62f8ce9f74b12f84c123cc23437a4a32';data = {"api_key":"882a8490361da98702bf97a021ddc14d","credentials_type":"password","email":id,"format":"JSON", "generate_machine_id":"1","generate_session_cookies":"1","locale":"en_US","method":"auth.login","password":pwd,"return_ssl_resources":"0","v":"1.0"};sig = 'api_key=882a8490361da98702bf97a021ddc14dcredentials_type=passwordemail='+id+'format=JSONgenerate_machine_id=1generate_session_cookies=1locale=en_USmethod=auth.loginpassword='+pwd+'return_ssl_resources=0v=1.0'+API_SECRET
	x = hashlib.new('md5')
        x.update(sig)
...
def post():
	...
	try:
	  if WT == 'wallpost':
		print '[*] tum yayinlarin kimligi getiriliyor'
		...
		for i in result['home']['data']:
			print '\r[*] %s alinan   '%(i['id']),;sys.stdout.flush();time.sleep(0.1)
		...
		for i in result['data']:
			print '\r[*] %s alinan    '%(i['from']['id']),;sys.stdout.flush();time.sleep(0.01)
		return result['data']
		...
		for i in result['friends']['data']:
			print '\r[*] %s alinan    '%(i['id']),;sys.stdout.flush();time.sleep(0.001)
		...
		for i in result['data']:
			print '\r[*] %s alinan    '%(i['id']),;sys.stdout.flush();time.sleep(0.01)
		return result

	  elif WT == 'albums':
		print '[*] tum albumler kimligi aliniyor'
		...
		print '[!] tum yayin kimligi alinamadi'
		print '[!] Durduruldu'
		bot()
	except requests.exceptions.ConnectionError:
		print '[!] Baglanti Hatasi'
...
def like(posts , amount):
	...
	print '\r[*] Tum gonderiler kimligi basariyla alindi            '
	print '[*] Baslat'
	...
	try:
		...
		for post in posts:
			...
			try:
				print '\r' + W + '[' + G + id + W + '] ' + post['mesaj'][:40].replace('\n',' ') + '...'
			except KeyError:
				try:
					print '\r' + W + '[' + G + id + W + '] ' + post['hikaye'].replace('\n',' ')
				except KeyError:
					print '\r' + W + '[' + G + id + W + '] Basariyla begenildi'
...
def comment(posts , amount):
	...
	try:
		...
		for post in posts:
			...
			try:
				print W + '[' + G + id + W + '] ' +post['mesaj'][:40].replace('\n',' ') + '...'
			except KeyError:
				try:
					print W + '[' + G + id + W + '] ' + post['hikaye'].replace('\n',' ')
				except KeyError:
					print W + '[' + G + id + W + '] basariyla yorumlandi'
		print '[*] Tamam'
		bot()
	except KeyboardInterrupt:
                print '\r[!] Durduruldu'
...
def remove(posts):
	...
	print '\r[*] Tum yayin kimligi basariyla alindi          '
	print '[*] Baslat'
	...
	try:
		...
		for post in posts:
			...
			try:
				cek = a['error']['message']
				print W + '[' + R + post['id'] + W +'] Basarisiz oldu'
			except TypeError:
				print W + '[' + G + post['id'] + W + '] Kaldırılan'
				counter += 1
		print '[*] Tamam'
		bot()
	except KeyboardInterrupt:
...
def confirm(posts):
	...
	print '\r[*] Tum arkadaslik istekleri basariyla alindi        '
	print '[*] Baslat'
	...
	try:
		...
		for post in posts:
			...
			try:
				cek = a['error']['message']
				print W + '[' + R + post['from']['name'] + W + '] Basarisiz oldu'
			except TypeError:
				print W + '[' + G + post['from']['name'] + W + '] Onaylanmis'
		print '[*] Tamam'
		bot()
	except KeyboardInterrupt:
		print '\r[!] Durduruldu'
...
#   [ OSM4N3R TARAFINDAN SIFRELER GIZLENMISTIR ! ]
...
def unfollow(posts):
	...
	print '\r[*] Tum kimlik basariyla alindi    '
	print '[*] Baslat'
	...
	try:
		...
		for post in posts['data']:
			...
			try:
				cek = a['error']['nessage']
				print W + '[' + R + post['name'] + W + '] Basarisiz oldu'
			except TypeError:
				print W + '[' + G + post['name'] + W + '] Vazgecebilirim'
		print '[*] Tamam'
		bot()
	except KeyboardInterrupt:
		print '\r[!] Durduruldu'
...
def poke(posts):
	...
	try:
		...
		for post in posts:
			...
			try:
				cek = a['error']['message']
				print W + '[' + R + id + W + '] Basarisiz oldu'
			except TypeError:
				print W + '[' + G + id + W + '] Durtmeler'
		print '[*] Tamam'
		bot()
	except KeyboardInterrupt:
		print '\r[!] Durduruldu   '
	...
	try:
		...
		for post in posts:
			...
			try:
				...
				print W + '[' + G + post['name'] + W + '] Cikarildi'
		print '[*] Tamam'
		bot()
	except KeyboardInterrupt:
		print '\r[!] Durduruldu  '
...
def menu_reaction_ask():
  try:
	...
	cek = raw_input(R + 'OSM4N3R' + W + '/' + R + 'Bot' + W + '/' + R + 'Reaction' + W + ' >> ')
	...
	if cek in ['1','01']:
		...
		try:
			...
			print '[!] bir erisim belirteci zaten var'
			cek = raw_input('[?] Devam etmek istediginizden emin misiniz [Y / N] ')
			if cek.lower() != 'y':
				print '[*] Iptal '
				bot()
		except IOError:
		...
		print '\n' + '[*] Access jetonu olustur facebook [*]'.center(44) + '\n'
		print '[DIKKAT] lutfen bu ozelligi kullanmadan once VPN nizi kapatin !!!'
		id()
	elif cek in ['0','00']:
		print '[!] bot menusune geri git'
		bot()
		...
		if cek == '':
			...
			print "[!] menu botunu gostermek icin 'menu' yazın"
			menu_reaction_ask()
  except KeyboardInterrupt:
	menu_reaction_ask()
...
def bot_ask():
	...
	print '[*] yuk erisim belirteci '
	try:
		token = open('cookie/token.log','r').read()
		print '[*] Basarili yukleme erisim belirteci'
	except IOError:
		print '[!] Yuk erisim belirteci basarisiz oldu'
		print "[!] erisim belirteci olusturmak icin 'token' yazin"
		menu_reaction_ask()
	...
	if WT.upper() == 'T':
		...
		if id == '':
			print "[!] kimlik hedefi bos olamaz"
			print '[!] Durduruldu'
			menu_reaction_ask()
...
def bot():
  try:
	...
	cek = raw_input(R + 'OSM4N3R' + W +'/' + R +'Bot ' + W + '>> ')
	...
	if cek in ['1','01']:
		...
		print '--------------------------------------------------'
		print "  [Not] Satiri degistirmek icin '</>' sembolunu kullanin \ n"

		message = raw_input('[?] Mesajin : ')
		if message == '':
			print "[!] Mesaj bos birakilamaz"
			print '[!] Durduruldu'
			bot()
		else:
		...
		if cek == '':
			...
			print "[!] komut '"+cek+"' bulunamadi"
			print '[!] menu botunu gostermek icin "menu" yazin'
			bot()
  except KeyboardInterrupt:
	bot()
#
...
def dump_id():
	...
	print '[*] Yuk Erisim Jetonu'
	try:
		token = open("cookie/token.log",'r').read()
		print '[*] basarili yuk erisim belirteci'
	except IOError:
		print '[!] basarisiz yukleme erisim belirteci'
		print "[*] erisim belirteci olusturmak icin 'token' yazin"
		main()
	...
	print '[*] tum arkadaslarin kimligi getiriliyor'
	try:
		...
		out.close()
		print '\r[*] tum arkadaslar kimlik basariyla alindi'
		print '[*] dosya kaydedildi : output/' + n[0].split(' ')[0] + '_id.txt'
		main()
		...
		print '[!] arkadas kimligi getirilemedi'
		main()
...
def dump_phone():
	...
	print "[*] tum telefon numaralarini getirme"
	print '[*] Baslat'
	...
	try:
		...
		for i in a['data']:
			...
			try:
				...
				print W + '[' + G + z['isim'] + W + ']' + R + ' >> ' + W + z['mobile_phone']
			except KeyError:
				pass
		out.close()
		print '[*] Tamam'
		print "[*] tum telefon numaralari basariyla alindi"
		print '[*] dosya kaydedildi : output/'+n[0].split(' ')[0] + '_phone.txt'
		main()
	except KeyboardInterrupt:
		print '\r[!] Durduruldu'
		main()
	except KeyError:
		print "[!] tum telefon numaralari getirilemedi"
		main()
	except (requests.exceptions.ConnectionError , requests.exceptions.ChunkedEncodingError):
		print '[!] Baglanti hatasi'
		print '[!] Durduruldu'
...
def dump_mail():
	...
	print '[*] tum e-postalar getiriliyor'
	print '[*] Baslat'
	...
	try:
		...
		for i in a['data']:
			...
			try:
                                out.write(z['email'] + '\n')
			        print W + '[' + G + z['isim'] + W + ']' + R + ' >> ' + W + z['email']
			except KeyError:
				pass
		out.close()

                print '[*] Tamam'
                print "[*] tum e-postalar basariyla alindi"
		print '[*] dosya kaydedildi : output/' + n[0].split(' ')[0] + '_mails.txt'
		main()
		...
		print "[!] tum e-postalar getirilemedi"
		main()
	except (requests.exceptions.ConnectionError , requests.exceptions.ChunkedEncodingError):
		print '[!] Baglanti hatasi'
		print '[!] Durduruldu'
...
def dump_id_id():
	...
	print '[*] arkadasinizdan tum kimligi getirme'
	...
	try:
		...
		print '\r[*] tum arkadaslar kimlik basariyla alindi'
		print '[*] dosya kaydedildi : output/' + n[0].split(' ')[0] + '_' + target_id + '_id.txt'
		main()
	except KeyboardInterrupt:
		print '\r[!] Durduruldu'
		main()
...
def main():
  ...
  try:
	cek = raw_input(R + 'OSM4N3R' + W +' >> ')

	if cek.lower() == 'get_data':
		if len(jml) == 0:
			getdata()
		else:
			print '[*] % S arkadas verisi aldiniz'%(len(jml))
			main()
	elif cek.lower() == 'get_info':
		print '\n'+'[*] Bilgi Toplama [*]'.center(44) + '\n'
		search()
	elif cek.lower() == 'bot':
		menu_bot()
		bot()
		...
		try:
			...
			print '[*] Erisim kartınız n !!\n\n' + o + '\n'
			main()
		except IOError:
			print '[!] Acilamadi cookie/token.log'
			print "[!] erisim belirteci olusturmak icin 'token' yazin"
			main()
		...
		print '\n' + '[*] Access jetonu olustur facebook [*]'.center(44) + '\n'
		print '[DIKKAT] lutfen bu özelligi kullanmadan once VPN nizi kapatın !!!'
		id()
	elif cek.lower() == 'rm_token':
		print '''
[DIKKAT] aşağıdaki durumlarda erişim belirteci oluşturmanız gerekir 
       erişim simgeniz silindi
'''
		a = raw_input("[!] devam etmek icin 'sil' yazin : ")
		if a.lower() == 'sil':
			try:
				os.system('rm -rf cookie/token.log')
				print '[*] Basarili silme cookie/token.log'
				main()
			except OSError:
				print '[*] Silinemedi cookie/token.log'
				main()
		else:
			print '[*] Silinemedi cookie/token.log'
			main()
	...
	if 'dump_' in cek.lower() and cek.lower().split('_')[2] == 'id':
		...
		if cek == '':
			...
			print "[!] komut '"+cek+"' bulunamadi"
			print '[!] komutu gostermek icin "help" yazin'
			main()
  except KeyboardInterrupt:
	main()
  except IndexError:
	print '[!] komutta gecersiz parametre : ' + cek
	main()
#
######################################################################################################################
...
def getdata():
	...
	print '[*] tum arkadas verilerini aliniyor'
	...
	try:
		...
		print '[!] Erisim simgenizin suresi doldu'
		print "[!] risim belirteci olusturmak icin 'token' yazin"
		main()
	...
	for i in a['data']:
		jml.append(i['id'])
		print '\r[*] fetching %s arkadaslardan veriler'%(len(jml)),;sys.stdout.flush();time.sleep(0.0001)

	print '\r[*] '+str(len(jml))+' arkadaslarin verileri basariyla alindi'
	main()

def search():

	if len(jml) == 0:
                print "[!] veritabaninda arkadas verisi yok"
                print '[!] arkadas verilerini toplamak icin "get_data" yazin'
                main()
        else:
                pass

	target = raw_input("[!] Adi veya Kimligi Ara : ")

	if target == '':
		print "[!] ad veya kimlik bos birakilamaz !!"
		search()
	else:
		info(target)
	...
	for i in a['data']:
	  ...
	  if target in  i['name'] or target in i['id']:
		...
		try:
			...
			pass
		try:
			print '[*] Kullanici adi : '+y['username']
		except KeyError:
			pass
		try:
			print '[*] Email : '+y['email']
			...
			pass
		try:
			print '[*] Telefon Numarasi : '+y['mobile_phone']
		except KeyError:
			pass
		try:
			print '[*] Isim : '+y['name']
			...
			pass
		try:
			print '[*] Ilk Isım : '+y['first_name']
		except KeyError:
			pass
		try:
			print '[*] Ikinci Ad : '+y['middle_name']
		except KeyError:
			pass
		try:
			print '[*] Soyadi : '+y['last_name']
		except KeyError:
			pass
		try:
			print '[*] Yerel : '+y['locale'].split('_')[0]
		except KeyError:
			pass
		try:
			print '[*] Yer : '+y['location']['name']
		except KeyError:
			pass
		try:
			print '[*] Memleket : '+y['hometown']['name']
		except KeyError:
			pass
		try:
			print '[*] Cinsiyet : '+y['gender']
		except KeyError:
			pass
		try:
			print '[*] Din : '+y['religion']
		except KeyError:
			pass
		try:
			print '[*] Iliski Durumu : '+y['relationship_status']
		except KeyError:
			pass
		try:
			print '[*] Siyasi : '+y['political']
		except KeyError:
			pass
		try:
			print '[*] Is :'
			...
			for i in y['work']:
				try:
					print '   [-] Durum : '+i['position']['name']
				except KeyError:
					pass
				try:
					print '   [-] Isveren : '+i['employer']['name']
				except KeyError:
					pass
				try:
					if i['start_date'] == "0000-00":
						print '   [-] Baslangic Tarihi : ---'
					else:
						print '   [-] Baslangic Tarihi : '+i['start_date']
				except KeyError:
					pass
				try:
					if i['end_date'] == "0000-00":
						...
						print '   [-] Bitis Tarihi : '+i['end_date']
				except KeyError:
					pass
				try:
					print '   [-] Yer : '+i['location']['name']
				except KeyError:
					pass
				print ' '
		except KeyError:
			pass
		try:
			print '[*] Guncelleme Zamani : '+y['updated_time'][:10]+' '+y['updated_time'][11:19]
		except KeyError:
			pass
		try:
			print '[*] Diller : '
		...
		try:
			...
			pass
		try:
			print '[*] Tirnak isareti : '+y['quotes']
		except KeyError:
			pass
		try:
			print '[*] Dogum Gunu : '+y['birthday'].replace('/','-')
		except KeyError:
			pass
		try:
			print '[*] Baglanti : '+y['link']
		except KeyError:
			pass
		try:
			print '[*] Favori Takimlar : '
